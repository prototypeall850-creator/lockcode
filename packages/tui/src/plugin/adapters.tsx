import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@lockcode-ai/plugin/tui"
...
import type { useLockcodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useLockcodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getLockcodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getLockcodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
