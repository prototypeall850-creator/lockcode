...
export const LOCKCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const LOCKCODE_MODE_KEY = "lockcode.mode"

export const LockcodeKeymapProvider = KeymapProvider
export const useLockcodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type LockcodeModeStack = ReturnType<typeof createLockcodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, LockcodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createLockcodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(LOCKCODE_MODE_KEY, LOCKCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(LOCKCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(LOCKCODE_MODE_KEY, stack.at(-1)?.mode ?? LOCKCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? LOCKCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(LOCKCODE_MODE_KEY, undefined)
...
export function useLockcodeModeStack() {
  return getLockcodeModeStack(useLockcodeKeymap())
}

export function getLockcodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Lockcode mode stack is not registered for this keymap")
  return value
}
...
export function registerLockcodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createLockcodeModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useLockcodeKeymap()
