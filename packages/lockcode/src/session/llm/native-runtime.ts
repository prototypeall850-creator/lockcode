...
import {
  ...
  type JsonSchema,
  type LLMEvent,
} from "@lockcode-ai/llm"
import type { LLMClientShape } from "@lockcode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("opencode"))
    return { type: "unsupported", reason: "provider is not openai, lockcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @lockcode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains lockcode-owned. The native runtime only adapts
      // the @lockcode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
