...
import { LockCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for lockcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(LockCodeHttpApi)
...
export const PublicApi = LockCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "lockcode",
    version: "1.0.0",
    description: "lockcode api",
