// Shared type vocabulary for the direct interactive mode (`lockcode --mini`).
//
...
import type { LockcodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@lockcode-ai/sdk/v2"
import type { TuiConfig } from "@lockcode-ai/tui/config"
...
type PromptModel = Parameters<LockcodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<LockcodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<LockcodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<LockcodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<LockcodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<LockcodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<LockcodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<LockcodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<LockcodeClient["question"]["reject"]>[0]
