import type { PermissionV1 } from "@lockcode-ai/core/v1/permission"
import { FSUtil } from "@lockcode-ai/core/fs-util"
// CLI entry point for `lockcode run` and `lockcode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`lockcode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`lockcode --mini --attach`): connects to a running
//      lockcode server and runs interactive mode against it.
//
...
import { createLockcodeClient, type LockcodeClient, type ToolPart } from "@lockcode-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<LockcodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run lockcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running lockcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to LOCKCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to LOCKCODE_SERVER_USERNAME or 'lockcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: LockcodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: LockcodeClient, sessionID: string) {
      ...
      async function current(sdk: LockcodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: LockcodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: LockcodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: LockcodeClient) {
        ...
        async function loop(client: LockcodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createLockcodeClient({
        baseUrl: "http://lockcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
