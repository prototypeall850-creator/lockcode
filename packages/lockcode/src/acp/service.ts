...
import type { AssistantMessage, Message, LockcodeClient, SessionMessageResponse } from "@lockcode-ai/sdk/v2"
...
export const AuthMethodID = "lockcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@lockcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `lockcode auth login` in the terminal",
      name: "Login with lockcode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: LockcodeClient) {
...
function makeUsageService(sdk: LockcodeClient) {
...
async function loadDirectorySnapshot(sdk: LockcodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, lockcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const lockcodeProvider = providers[ProviderV2.ID.make("opencode")]
  const lockcodeModel = lockcodeProvider ? Provider.sort(Object.values(lockcodeProvider.models))[0] : undefined
  if (lockcodeProvider && lockcodeModel) return { providerID: lockcodeProvider.id, modelID: lockcodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "LockCode service failure", service })
}
