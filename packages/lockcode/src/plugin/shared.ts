...
export const DEPRECATED_PLUGIN_PACKAGES = ["lockcode-openai-codex-auth", "lockcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, lockcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(lockcodeVersion) || semver.major(lockcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.lockcode
  if (typeof range !== "string") return
  if (!semver.satisfies(lockcodeVersion, range)) {
    throw new Error(`Plugin requires lockcode ${range} but running ${lockcodeVersion}`)
  }
}
