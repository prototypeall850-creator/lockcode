...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@lockcode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@lockcode-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@lockcode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@lockcode-ai/tui/context/theme"
import { Global } from "@lockcode-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@lockcode-ai/core/util/flock"
import { Flag } from "@lockcode-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@lockcode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@lockcode-ai/core/v1/config/plugin"
import { createCommandShim } from "@lockcode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@lockcode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".lockcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".lockcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".lockcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.LOCKCODE_PURE ? [] : pluginOrigins
    if (Flag.LOCKCODE_PURE && pluginOrigins.length) {
    }
