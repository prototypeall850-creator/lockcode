...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from lockcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const lockcode = await lockcodeFiles(input)
  for (const file of lockcode) {
...
async function lockcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "lockcode"),
    ...(await Filesystem.findUp(["lockcode.json", "lockcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "lockcode"))
  }
  if (Flag.LOCKCODE_CONFIG) files.push(Flag.LOCKCODE_CONFIG)
