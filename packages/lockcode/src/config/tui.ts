...
import { isRecord } from "@lockcode-ai/tui/util/record"
...
import { TuiKeybind } from "@lockcode-ai/tui/config/keybind"
...
import { TuiConfig } from "@lockcode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@lockcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old lockcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.lockcode`
  // folders between cwd and home, and LOCKCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.LOCKCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit LOCKCODE_TUI_CONFIG override, if set.
  if (Flag.LOCKCODE_TUI_CONFIG) {
    const configFile = Flag.LOCKCODE_TUI_CONFIG
  ...
  // 4. `.lockcode` directories (and LOCKCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".lockcode") || dir === Flag.LOCKCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".lockcode") && dir !== Flag.LOCKCODE_CONFIG_DIR) continue
