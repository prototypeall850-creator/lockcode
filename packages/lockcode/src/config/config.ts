...
import { httpClient } from "@lockcode-ai/core/effect/app-node-platform"
import { serviceUse } from "@lockcode-ai/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@lockcode-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@lockcode-ai/core/v1/config/console-state"
import { FSUtil } from "@lockcode-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@lockcode-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@lockcode-ai/core/v1/config/config"
import { RemoteAuthError } from "@lockcode-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@lockcode-ai/core/v1/config/permission"
import { ConfigPluginV1 } from "@lockcode-ai/core/v1/config/plugin"
...
import { Npm } from "@lockcode-ai/core/npm"
...
export class Service extends Context.Service<Service, Interface>()("@lockcode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["lockcode.jsonc", "lockcode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.LOCKCODE_CONFIG && !Flag.LOCKCODE_CONFIG_DIR && !Flag.LOCKCODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "lockcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "lockcode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "LOCKCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/lockcode`
        ...
        if (Flag.LOCKCODE_CONFIG) {
          yield* merge(Flag.LOCKCODE_CONFIG, yield* loadFile(Flag.LOCKCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.LOCKCODE_CONFIG })
        }

        if (!Flag.LOCKCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("lockcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.LOCKCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from LOCKCODE_CONFIG_DIR", { path: Flag.LOCKCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".lockcode") || dir === Flag.LOCKCODE_CONFIG_DIR) {
            for (const file of ["lockcode.json", "lockcode.jsonc"]) {
          ...
          const dep = yield* npmSvc
            .install(dir, {
              add: [
                {
                  name: "@opencode-ai/plugin", // published npm name (runtime dep untuk user plugin)
          ...
          // Auto-discovered plugins under `.lockcode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.LOCKCODE_CONFIG_CONTENT) {
          const source = "LOCKCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.LOCKCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from LOCKCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["LOCKCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("LOCKCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.LOCKCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LOCKCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("LOCKCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.LOCKCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.LOCKCODE_DISABLE_PRUNE) {
