...
Practical reference for a future `packages/server` split after the lockcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/lockcode`.
...
Until enough shared service code lives outside `packages/lockcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/lockcode`

It should not import `packages/lockcode` services while `packages/lockcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on lockcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/lockcode` implementation details.
