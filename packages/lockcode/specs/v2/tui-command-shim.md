...
Delete `packages/lockcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/lockcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/lockcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/lockcode`
- TUI plugin loader tests in `packages/lockcode` if runtime plugin API wiring changed
