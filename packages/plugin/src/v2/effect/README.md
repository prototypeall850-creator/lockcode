# LockCode V2 Effect Plugin API

The Effect plugin API grants plugins two in-process capabilities:

- `hook` installs behavior at an LockCode extension point.
...
```ts
import { define } from "@lockcode-ai/plugin/v2/effect"
...
LockCode rebuilds the domain when a transform is registered or disposed. A rebuild starts from fresh domain state and runs every active transform in registration order.
