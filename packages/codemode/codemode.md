...
This is the living design and status document for `@lockcode-ai/codemode` and its existing V2 LockCode adapter.
...
`packages/lockcode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
