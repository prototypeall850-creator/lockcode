...
import { SessionInput } from "@lockcode-ai/schema/session-input"
import { PromptInput } from "@lockcode-ai/schema/prompt-input"
import { Session } from "@lockcode-ai/schema/session"
import { Project } from "@lockcode-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@lockcode-ai/schema/schema"
import { Workspace } from "@lockcode-ai/schema/workspace"
...
import { Agent } from "@lockcode-ai/schema/agent"
import { Model } from "@lockcode-ai/schema/model"
import { Location } from "@lockcode-ai/schema/location"
import { Revert } from "@lockcode-ai/schema/revert"
import { SessionEvent } from "@lockcode-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this LockCode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this LockCode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
