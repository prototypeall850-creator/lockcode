...
import { ToolFailure } from "@lockcode-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@lockcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.LOCKCODE_WEBSEARCH_PROVIDER === "exa" || process.env.LOCKCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.LOCKCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("LOCKCODE_EXPERIMENTAL") || truthy("LOCKCODE_ENABLE_EXA") || truthy("LOCKCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("LOCKCODE_ENABLE_PARALLEL") || truthy("LOCKCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `lockcode/${InstallationVersion}`,
