...
import type { IntegrationOAuthMethodRegistration } from "@lockcode-ai/plugin/v2/effect/integration"
import { define } from "@lockcode-ai/plugin/v2/effect/plugin"
...
const clientID = "opencode-cli" // server-side registered OAuth client id (external)
...
function oauth(http: HttpClient.HttpClient) {
  return {
    ...
    method: {
      id: methodID,
      type: "oauth",
      label: "LockCode Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
...
export const LockcodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "lockcode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("LockcodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("lockcode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load LockCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("lockcode", (integration) => {
        integration.name = "LockCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "lockcode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("lockcode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      ...
      const item = catalog.provider.get(ProviderV2.ID.lockcode)
      if (!item) return
