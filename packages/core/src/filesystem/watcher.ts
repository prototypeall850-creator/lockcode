...
import { FileSystemWatcher } from "@lockcode-ai/schema/filesystem-watcher"
...
declare const LOCKCODE_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof LOCKCODE_LIBC === "undefined" ? undefined : LOCKCODE_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@lockcode/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.LOCKCODE_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.LOCKCODE_EXPERIMENTAL_FILEWATCHER)) {
